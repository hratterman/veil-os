...
scripts/build.sh >/tmp/veil-m38-build.log 2>&1 && cargo build --release >>/tmp/veil-m38-build.log 2>&1 || { echo "BUILD FAIL"; tail -20 /tmp/veil-m38-build.log; exit 2; }
scripts/mkdisk.sh >/dev/null 2>&1 || exit 2

KERNEL=target/aarch64-unknown-none/release/veil
QMP=/tmp/veil-m38-qmp.sock
SERIAL=/tmp/veil-m38-serial.log
mkdir -p shots
rm -f "$QMP" "$SERIAL"
