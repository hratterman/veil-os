...
fn virt_main(dtb_ptr: *const u8) -> ! {
    ...
    if !fastboot(&fdt) {
        ...
        js::react_full_selftest(); // M42 step 17: REAL React 18 (createRoot) mounts a stateful app + onClick re-renders -> REACT_FULL_OK
...
fn milestone4() {
    ...
    const HEAP_FRAMES: usize = 49152; // 192 MiB — the React 18 dev build (REACT_FULL_OK) needs the headroom
